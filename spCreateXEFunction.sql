		...
		*/
		...
		INSERT	INTO @EventColumns
				SELECT	e.EventName, c.name, c.type_name, c.column_type, 
						...
						JOIN	sys.dm_xe_packages p
							ON	p.guid = c.object_package_guid 
							AND	(p.guid = e.PackageGUID OR p.name = 'package0')
							...
							*/
							...
							OR	 EXISTS (
								SELECT	1 FROM sys.server_event_session_fields o
										...
										AND		o.value = 1))
							/*
							**	If an event field and a global field have the same name, query the global field
							*/
							AND	NOT EXISTS (
								SELECT	1 FROM @EventColumns ec
										WHERE	ec.ColName = c.name
										AND		ec.EventName = e.EventName)
		SELECT	@Cols = @Cols + @@ROWCOUNT
		IF		@Cols = 0
